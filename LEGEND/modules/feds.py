...
from LEGEND import DRAGONS, EVENT_LOGS, LOGGER, OWNER_ID, TIGERS, WOLVES, dispatcher, LEGENDX22
...
def del_fed(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        if getinfo is False:
            ...
            return
        if int(getinfo["owner"]) == int(user.id) or int(user.id) == OWNER_ID or int(user.id) == LEGENDX22:
            fed_id = is_fed_id
        else:
